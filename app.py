...
# Sidebar - Profile
with st.sidebar:
    # Gunakan emoji sebagai placeholder untuk gambar
    st.markdown("# 📊")
    st.markdown("### 👤 **Profil**")
...
def load_data():
    try:
        ...
        st.error("File churn.csv tidak ditemukan. Pastikan file ada di direktori yang sama dengan app.py")
        st.stop()
...
if page == "📈 Overview":
    ...
    with col2:
        ...
        # Handle case where 'Churn' column might not exist
        if 'Churn' in df.columns:
            st.metric("Churn Rate", f"{(df['Churn']=='Yes').mean():.1%}")
        else:
            st.error("Column 'Churn' tidak ditemukan dalam dataset")
    ...
    required_cols = ['Churn', 'Gender', 'MonthlyCharges', 'Tenure', 'Contract']
    ...
    available_features = ['Gender', 'Partner', 'Dependents', 'Contract', 
    ...
    if available_features:
        ...
        st.warning("No categorical features available for analysis")
    ...
    with col3:
        # Exclude 'customerID' if exists, otherwise just exclude 'Churn'
        exclude_cols = ['Churn']
        if 'customerID' in df.columns:
            exclude_cols.append('customerID')
        total_features = len(df.columns) - len(exclude_cols)
    ...
    try:
        ...
        if 'customerID' in df.columns:
            exclude_columns.append('customerID')
        
        X = df.drop(exclude_columns, axis=1).copy()
        y = df['Churn'].copy()
        ...
        for col in categorical_features:
            ...
            try:
                ...
                test_values = X_test_processed[col].astype(str).values
                encoded_values = []
    ...
    # Conditional ROI calculation based on available columns
    try:
        total_customers = len(df)
        col1, col2, col3, col4 = st.columns(4)
        ...
        if 'Churn' in df.columns and 'MonthlyCharges' in df.columns:
            ...
            with col2:
                st.metric("Monthly Churn Cost", "Data N/A")
            with col3:
                st.metric("Annual Churn Cost", "Data N/A")
            with col4:
                st.metric("Potential Savings", "Data N/A")
                
    except Exception as e:
        st.warning("Unable to calculate ROI metrics - some data columns may be missing")
